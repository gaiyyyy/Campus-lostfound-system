    ...
    <el-header class="main-header">
      <div class="header-content">
        <div class="logo-section">
          ...
          <div class="logo-text">
            <h1 class="logo-title">招领列表</h1>
            <p class="logo-subtitle">Found Items</p>
          </div>
        </div>
        <div class="header-tools">
          ...
          <el-tooltip content="发布新招领" placement="bottom" v-if="currentUsername">
      ...
      <el-card class="content-card" shadow="hover">
        ...
        <div class="filter-section">
          ...
          <div class="filter-container">
            ...
            <div class="filter-stats" v-if="!loading && list.length > 0">
              <span class="stat-text">共 {{ list.length }} 条招领信息</span>
              <span class="stat-text" v-if="hasFilters">
                ，筛选后显示 {{ filteredCount }} 条
              </span>
            </div>
          </div>
        </div>
        ...
        <div class="table-section">
          ...
          <div class="table-container">
            ...
            >
                  ...
                  <div class="item-title">
                    ...
                    <div class="item-info">
                      ...
                      <div class="item-meta">
                        ...
                        <span class="item-time">{{ formatTime(row.foundTime) }}</span>
                      </div>
                    </div>
                  </div>
                </template>
              ...
              <el-table-column label="拾到地点" width="150">
                <template #default="{ row }">
                  <div class="location-cell">
                    <el-icon :size="14" style="margin-right: 4px;"><Location /></el-icon>
                    <span>{{ row.foundLocation || '未填写' }}</span>
                  </div>
                </template>
              </el-table-column>
              ...
              <el-table-column label="发布者" width="130">
                <template #default="{ row }">
                  <div class="publisher-cell">
                    <el-avatar :size="28" :style="{ backgroundColor: getAvatarColor(row.publisherName) }">
                      {{ getPublisherInitial(row.publisherName) }}
                    </el-avatar>
                    <span class="publisher-name">{{ row.publisherName || '匿名' }}</span>
                  </div>
                </template>
              </el-table-column>
...
import { 
  Present, HomeFilled, Plus, Search, 
  Delete, List, Box, Location, View 
...
const emptyText = computed(() => {
  ...
  if (hasFilters.value) return "没有找到匹配的招领信息";
  return "暂无招领信息";
});
...
const filteredList = computed(() => {
  ...
  if (searchKeyword.value.trim()) {
    ...
    filtered = filtered.filter(item => 
      ...
      (item.foundLocation && item.foundLocation.toLowerCase().includes(keyword))
    );
  }
  ...
  return filtered.sort((a, b) => {
    return new Date(b.foundTime || b.createTime) - new Date(a.foundTime || a.createTime);
  });
});
...
const goDetail = (id) => router.push(`/found/${id}`);
const goNew = () => router.push("/found/publish");
